...
func add_crop() -> void:
	if distance < 20.0 and cell_source_id != -1:
		# Check if there is already a crop at this position
		var crop_nodes = get_parent().find_child("CropFields").get_children()
		for node: Node2D in crop_nodes:
			if node.global_position == local_cell_position:
				return # A crop already exists here, do nothing

		# Only allow planting if the tile has a valid source ID (tilled soil)
