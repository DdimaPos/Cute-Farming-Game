...
func _on_physics_process(_delta : float) -> void:
	#create a new route if chicken reached the final objective
	...
	# tutorial 10
	# probably the algorithms can be included here
	var target_direction: Vector2 = character.global_position.direction_to(target_position)
	
	
	var velocity: Vector2 = target_direction * speed;
	
	# logic about preventing mobs colliding with each other if avoidance enabled
	if navigation_agent_2d.avoidance_enabled:
		# mirror the animation if he moves to left
